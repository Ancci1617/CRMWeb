...
async function cargarEfectivo(req, res) {
    ...
    try {
        const [efectivo] = await pool.query("UPDATE `PlanillasDeCobranza` SET EFECTIVO = ? WHERE ID = ? AND EDITABLE = 1", [EFECTIVO, ID]);
    } catch (error) {
        console.log(error);
...
async function rendicionReceptor(req, res) {
    ...
    const [rendicion] = await pool.query(
        "SELECT Pl.ID,`COB`, Pl.`FECHA`, `EFECTIVO`, `RECEPCION`, `EDITABLE`, SUM(Pa.VALOR + Pa.MORA + Pa.SERV) AS TOTAL_COBRADO, SUM(CASE WHEN Pa.MP_OPERACION IS NOT NULL > 0 THEN Pa.MP ELSE 0 END) as MP, IFNULL((SELECT SUM(MONTO) from Gastos where ID_RENDICION = Pl.ID),0) AS TOTAL_GASTOS , SUM(Pa.VALOR + Pa.MORA + Pa.SERV) - SUM(CASE WHEN Pa.MP_OPERACION IS NOT NULL > 0 THEN Pa.MP ELSE 0 END) - IFNULL((SELECT SUM(MONTO) from Gastos where ID_RENDICION = Pl.ID),0) - EFECTIVO as DIFERENCIA FROM `PlanillasDeCobranza` Pl Left join PagosSV Pa on Pa.FECHA = Pl.FECHA AND Pa.COBRADOR = Pl.COB WHERE Pl.FECHA = ? and Pl.COB = ?;", [FECHA, COB]);
