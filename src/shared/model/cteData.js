...
const getPagosAcumulados = async ({ CTE, Easy = false }) => {
    ...
    FROM (SELECT * from PagosSV where PagosSV.CONFIRMACION != 'INVALIDO' union select * from PagosSVAcumulado) pm LEFT JOIN BaseDetalle bd on pm.CTE = bd.CTE and pm.FICHA = bd.FICHA left join Fichas f on f.CTE = pm.CTE and f.FICHA = pm.FICHA WHERE pm.CTE IN (?) ${Easy ? " AND pm.FICHA > 50000" : ''} AND (bd.ESTADO = "ACTIVO" or bd.ESTADO is null) order by CODIGO,pm.FECHA;
    `, [CTE])
