...
async function updatePedidoByID({ CTE, ZONA, NOMBRE, CALLE, CRUCES, CRUCES2, TELEFONO, QUE_NECESITA, DIA_VISITA, DESDE, HASTA, DESIGNADO,  MOTIVO = "", ESTADO = "PENDIENTE" }, ID) {


    const [update] = await pool.query(
        "UPDATE `Pedidos` SET `CTE`=? , `ZONA`=? , `NOMBRE`=? , `CALLE`=?, " +
        "`CRUCES`=?,`CRUCES2`=?,`TELEFONO`=?,`QUE_NECESITA`=?,`DIA_VISITA`=?, " +
        "`DESDE`=?,`HASTA`=?,`DESIGNADO`=?,`MOTIVO`=?,`ESTADO`=? WHERE ID = ?"

        , [CTE, ZONA, NOMBRE, CALLE, CRUCES, CRUCES2, TELEFONO, QUE_NECESITA, DIA_VISITA, DESDE, HASTA, DESIGNADO, MOTIVO, ESTADO, ID]);
