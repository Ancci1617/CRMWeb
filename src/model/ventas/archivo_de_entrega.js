...
async function getArchivoDeEntregaVentas(vendedor,dia) {
    const [ventas] = await pool.query(
        ...
        "Concat(year(PRIMER_PAGO),'-',Month(PRIMER_PAGO),'-',VENCIMIENTO) as VENCIMIENTO from VentasCargadas where " + 
        "USUARIO = ? and FECHA_VENTA = ? and VISIBLE = 1 order by FICHA", [vendedor,dia])
