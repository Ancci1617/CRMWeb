...
async function getArchivoDeEntregaVentas(vendedor, dia) {
    const [ventas] = await pool.query(
        "Select FECHA_VENTA, CTE, FICHA, ZONA, TOTAL, null as n1, "+
        "null as n2, 'formula' as n3, 'formula' as n4, CUOTA, null as n5, 'formula' as n6, "+
        "'formula' as n7, 'formula' as n8, tipo, ESTATUS, PRIMER_PAGO, NULL as n9, NULL as n10, "+
        ...
        "if (dateDiff(Concat(year(FECHA_VENTA), '-', Month(FECHA_VENTA) + 1, VENCIMIENTO), FECHA_VENTA) >= 32, "+
        "Concat(year(FECHA_VENTA), '-', Month(FECHA_VENTA), '-', VENCIMIENTO), Concat(year(FECHA_VENTA), '-', "+
        "Month(FECHA_VENTA) + 1, '-', VENCIMIENTO)) as VENCIMIENTO " + 
        "from VentasCargadas where USUARIO = ? and FECHA_VENTA = ? and VISIBLE = 1 order by FICHA;", [vendedor, dia])
